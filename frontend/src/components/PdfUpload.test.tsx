...
describe("PdfUpload", () => {
  ...
  it("does not set the multiple attribute by default", () => {
    render(<PdfUpload label="Upload a PDF" onExtracted={vi.fn()} />);
    const input = screen.getByLabelText(/upload a pdf/i, { selector: "input" }) as HTMLInputElement;
    expect(input.multiple).toBe(false);
  });

  it("extracts each selected file and calls onExtracted once per file, in order, when multiple is set", async () => {
    mockExtract.mockImplementation((file: File) => Promise.resolve(`text from ${file.name}`));
    const onExtracted = vi.fn();
    render(<PdfUpload label="Upload PDFs" onExtracted={onExtracted} multiple />);

    const input = screen.getByLabelText(/upload pdfs/i, { selector: "input" }) as HTMLInputElement;
  ...
  it("still extracts the files that succeed when one file in a multi-select fails", async () => {
    mockExtract.mockImplementation((file: File) =>
